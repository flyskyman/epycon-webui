...
仓库包含一个可配置的生成脚本： scripts/generate_fake_wmx.py，用于生成 WMx64（`--version 4.3`，真实记录全为 4.3.2）或 WMx32（`--version 4.1`）的占位二进制日志（仅用于测试流程，非真实临床数据）。示例用法：

```powershell
# 生成单通道、1024 samples 的默认文件
python scripts\generate_fake_wmx.py --version 4.3

# 生成两通道、512 samples、采样率 500Hz 的文件到指定位置
python scripts\generate_fake_wmx.py --out examples/data/study01/00000001.log --channels 2 --samples 512 --value 500 --fs 500 --version 4.3
```
