                ...
                for field in ("fids", "groups"):
                    if field in criteria:
                        ...
                        if isinstance(val, str):
                            criteria[field] = {val}  # type: ignore[assignment]
                        elif isinstance(val, (list, tuple)):
                            criteria[field] = set(val)  # type: ignore[arg-type]
...
class DatalogPlanter:
    def __init__(
        ...
        **kwargs,
        ...
        self.f_path = f_path
        self._f_obj = None
        # Extract extension, handle both str and bytes paths
        _ext = os.path.splitext(f_path)[1]
        if isinstance(_ext, bytes):
            _ext_str = _ext.decode('utf-8')
        else:
            _ext_str = str(_ext)
        self._extension = _validate_str("output file extension", _ext_str.lower(), valid_set={".csv", ".h5"})
...
class HDFPlanter(DatalogPlanter):
    ...
    def write(
        ...
        if not self._header_isstored:
            ...
            # Ensure items are str before joining/encoding
            _str_names: List[str] = [
                (item.decode('utf-8') if isinstance(item, bytes) else str(item))
                for item in self.column_names
            ]
            self.column_names = [''.join(name.split()).encode('UTF-8') for name in _str_names]
    ...
    def _generate_channel_info(
            ...
            if isinstance(self.units, (tuple, list)):
                # Encode each unit if it's a string, otherwise keep bytes
                self.units = [
    ...
    def add_samples(
        ...
        # Access as Dataset (h5py typing workaround)
