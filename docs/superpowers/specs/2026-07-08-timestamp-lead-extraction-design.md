...
1. 按目标流逝时刻**定位覆盖段**（在各段半开区间 `[ts, ts+dur)` 中反查，与第 4 节一致）
2. ±窗口**裁剪到段界**并如实报告
3. **栏杆（未连接）检测**
...
- **时间解析约定**：WorkMate 屏幕显示整秒，但段起点是毫秒级（如 `1:07:12.342`）。
  故整秒目标**按字面值**解析（`1:07:12` = 恰 4032.0s，不做四舍五入到最近段），
...
3. 每条 entry 的 epoch 必须落在其 `fid` 对应 log 的半开区间 `[start, start+dur)` 内
   （与第 3、4 节统一，避免段终点归属歧义）。
...
- **显示名 ≠ 独立信号**（issue #27）：WorkMate 通道表可把同一电极对登记为多个显示名
  （`LBB` / `LBB-I` / `LBB 30-100` …，只是屏幕滤波带不同），原始 `.log` 只有一份采样。
  `--leads all` 按源列去重、只出首个名字并在 `aliases` 列出其余；显式点名不合并。
...
- 版本：`--version` 显式，缺省取 config `workmate_version`。**无自动探测**
  （自动探测会静默误采纳，违反 fail-closed）。显式传错版本（如 x32 套 x64）
  属用户误用、由用户负责（见第 12 节），不在工具的 fail-closed 边界内。
...
- **原始整数读取路径**：`LogParser.read()` 经 `_process_chunk`（`_twos_complement` + ×resolution）
  ...
  **`raw_int` 必须保持 Python int 或 int64，严禁强转 int32**——栏杆解析产物 `-2147483649`
  = `-(2³¹)-1` 越出 int32 范围，强转会溢出。Codex 已核实：realdata 全 12 段所有样本
  均为 78 的整数倍且 `round(scaled/78)` 无损；上界 `(2³¹+1)×78 ≈ 1.675e11 < 2⁵³`，
  float64 精确整数范围内。tests 须钉死"栏杆样本还原为 `-2147483649` 且不溢出"。
