...
### 30. 单个坏 entry 让整个 study 无法导出（#23 的实证，且波形被连坐）
- **实测面**：外部批量使用中，52 个 study 有 **6 个（11.5%）完全导不出**，两个 CLI 入口都失败。
  波形数据本身没有问题，问题全在 `entries.log` 的个别条目。
- **两类原因**：
  1. **未初始化哨兵时间戳**（4/6，全部 `fid=00000000`）：epoch 恒为 `1.8446744073709…e+16`，
     即 `2^64/1000`。这不是"边界过严"，而是垃圾值——`check_consistency` 对它没有专门判别。
  2. **亚秒级边界差**（2/6，时间戳正常）：一例 entry 比其段起点早 **0.129 s**，
     一例比其段终点晚 **0.101 s**。段本身长达数百秒。
- **与 #23 的关系**：#23（2026-07-08）把整簿硬断言按 by-design 关闭，并写明重开条件是
  "若未来遇到末端标注误报"。上述第 2 类正是该实证（且是**段首**方向，不只是末端半开上界）；
  第 1 类是 #23 未覆盖的新情形。建议据此重开评估"仅校验目标段"。
- **波形被连坐（本条最要紧、#23 未涉及）**：同一哨兵值也会让批量转换器崩溃：
  ```
  File "epycon/core/_formatting.py", line 114, in _tocsv
      dtime = datetime.fromtimestamp(item.timestamp)
  ValueError: year must be in 1..9999, not 584556019
  ```
  调用链 `__main__.py:116` → `iou/planters.py:60 savecsv` → `_formatting.py:114`。
  **崩溃时输出目录是 0 字节**——波形 h5 一个字节都没写。波形转换与 entries 无关，
  却被一个坏条目整体归零。把 `entries.convert` / `entries.summary_csv` / `data.pin_entries`
  全关掉后同一 study 正常导出（15 段合并、6,350,750 样本、688 MB）。
- **建议处置**（按性价比排序）：
  1. **波形与 entries 解耦**：entries 失败不阻止波形落盘——单这一条就能把这 6 个 study
     从"完全不可用"变成"可用但无标注"；
  2. `_tocsv` 的 `datetime.fromtimestamp` 加保护，异常条目输出原始整数而非抛出；
  3. `check_consistency` 把"epoch 非有限 / 超 `datetime` 可表示范围 / 命中 2^64 量级哨兵"
     判为无效条目（丢弃 + 计数告警），与"边界过严"分开处理；
  4. 边界比较加容差或仅校验目标段（即 #23 的重开选项）。
- **来源**：2026-08-25，外部项目按墙钟时刻批量提取 53 例宽带窗口时发现


## 中优先级

### 31. `computed` 双极极性相对设备显示与起搏伪差方向翻转（请重新评估 #16）
- **一手实测 1 — 合成方向**：`computed` 严格等于 **u− − u+**（同一窗口、raw counts、
  整数完全相等，非近似）。
- **一手实测 2 — 与设备显示相反**：25 份同时拥有 WorkMate 显示导出与原始 `.log` 的记录中，
  **23 份**导出腔内通道与 epycon `computed` 对应导联呈负相关，r = −0.93 … −0.995
  （按该通道登记滤波带重建后比较，起搏伪差 ±10 ms 已遮蔽）；2 份为正、原因未查。
  同批记录**体表导联增益是 +1**，排除比较方一侧的全局符号错误。
- **一手实测 3 — 物理判据（决定性）**：在**存在真实起搏伪差**（中位幅度 0.47–2.36 V）的
  15 份记录中，**导出通道的起搏伪差 15/15 为负向**（一份 0.93）。阴极起搏在施放电极上
  产生负向伪差是物理常规，故**设备显示这一侧与物理常规一致，epycon 输出是翻转的**
  （其起搏伪差为正向）。
  （另有 10 份被本次统计标为"正向"的记录，其极值幅度仅 7–27 mV，并非起搏伪差，
  是选通道时挑到了未施放刺激的通道，不构成反证。）
- **与 #16 的关系——建议重开**：#16（2026-07-16）判定"代码对、标签错"，其依据是
  单极通道名/`pin` 的编号算术，并明确写"不需要 WorkMate 屏幕对照"。
  本条复核了它的前提：header 的 `pin` 字段确实呈 u−=低针脚、u+=高针脚的稳定模式
  （如 CS 十极：CS d 40/41、CS 3-4 42/43、CS 5-6 44/45、CS 7-8 46/47、CS p 48/49），
  该模式对**固定接线的诊断导管**成立。但**起搏导线经 JBox 接线时，tip/ring 落在哪个
  针脚是术中接线选择，不由编号决定**，因此编号算术无法判定起搏导线的双极方向——
  而本项目关心的正是起搏导线通道。起搏伪差极性是不依赖接线假设的物理判据，
  它与 #16 的结论方向相反。建议以波形证据重新评估 #16。
- **影响**：拿 epycon 输出与 WorkMate 屏幕/导出对照时，腔内双极信号上下翻转；
  幅度与时间都对、只有符号反，形态学判读（电位方向、S 波深度、极性、伪差方向）
  会**静默出错**。体表导联不受影响，因此更难被发现。
- **建议处置**：
  1. 用起搏伪差极性复核 #16（成本低、判据明确：阴极刺激应为负向）；
  2. 若确认，翻转 `computed` 合成方向或提供显式开关——属 user-visible change，进 Release notes；
  3. 无论是否改默认值，都应在输出契约/文档写明当前方向及其相对设备显示的关系。
- **未决**：2/25 呈正相关的记录未查明原因（可能该导出通道自身带反相标记）。
- **来源**：2026-08-25，外部项目批量提取 53 例宽带窗口时发现并复核

## 低优先级

### 32. `extract` 导联解析全有或全无；缺 `--leads all`；通道表逐段不同未在文档说明
- **全有或全无**：`--leads` 名字须精确匹配，任一个无效（不在通道表 / 源电极参考无效）
  整个调用即失败，已能解析的导联也拿不到。`is_railed` 已经是 per-lead 的
  `status: rejected`，但名字解析失败（`extraction.py:145 resolve_lead_sources`）是致命的。
  建议统一成 per-lead status。
- **缺 `--leads all`**：批量场景下调用方必须自己先取通道表再拼字符串。
- **通道表逐段不同（文档未提示）**：同一 study 不同 `.log` 段的可用导联不一样；
  用 `segs[0].header` 取通道表再去提取别的段会报"导联 X 不在通道表"，必须按**目标段**解析。
  `docs/` 与 CLI help 均未说明，批量调用容易踩。
- **次要**：`--epoch` 传浮点需足够精度（`repr(float)`），help 未说明；`--out` 相对路径
  按进程 cwd 解析，从其他目录调用易写到意料之外位置。
- **来源**：2026-08-25，同上批量提取
