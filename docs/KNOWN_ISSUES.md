...
### 31. `computed` 双极极性与 WorkMate 显示导出相反（与 #16 并列，不是推翻）
- **实测**：25 份同时拥有 WorkMate 显示导出（逐导联 TXT）与原始 `.log` 的记录中，**23 份**
  的导出腔内通道与 epycon `computed` 对应导联呈**负相关**，r = −0.93 … −0.995
  （把宽带原始信号按该通道登记的滤波带重建后比较，起搏伪差 ±10 ms 已遮蔽）；2 份为正。
  同批记录的**体表导联增益是 +1**，故不是比较方一侧的全局符号错误。
- **与 #16 的关系**：#16（2026-07-16）用单极通道编号算术证明 `computed` 合成
  **符合 EP 惯例**（第一个电极减第二个），并明确说"不需要 WorkMate 屏幕对照"。
  本条测的正是屏幕对照。两者若都成立，推论是：**WorkMate 显示/导出的腔内双极极性
  与 EP 惯例相反**，而不是 epycon 算错。因此**不建议改默认合成方向**。
- **为什么仍要入账**：任何拿 epycon 输出与 WorkMate 屏幕/导出对照的人，腔内双极信号是
  上下翻转的。幅度和时间都对、只有符号反，做形态学判读（电位方向、S 波深度、极性）
  会**静默出错**；体表导联不受影响，所以更不容易被发现。
- **建议处置**：在输出契约/文档里显式写明"腔内双极极性与设备显示相反，与 EP 惯例一致"，
  或提供一个 `--match-display` 开关。不改默认值。
- **待确认**：2/25 为正的两份记录原因未查（可能该导出通道自身带反相标记）。
